...
export async function stratTestTransferToken() {
  ...
  const startBalance = await wmasToken.balanceOf(multisigContract.address);
  ...
  const balance = await wmasToken.balanceOf(multisigContract.address);
  ...
  const profitBalanceBef = await wmasToken.balanceOf(profitAddr);
  ...
  const profitBalanceAft = await wmasToken.balanceOf(profitAddr);
